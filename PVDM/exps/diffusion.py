...
def diffusion(rank, args):
    ...
    ema_model = None
    if args.n_gpus > 1:
        first_stage_model = torch.nn.parallel.DataParallel(first_stage_model)
        model = torch.nn.parallel.DataParallel(model)
        model = model.module
        first_stage_model = first_stage_model.module
    ...
    if args.n_gpus > 1:
        criterion = torch.nn.parallel.DataParallel(criterion)
