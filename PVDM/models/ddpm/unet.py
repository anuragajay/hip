...
class DiffusionWrapper(nn.Module):
    ...
    def forward(self, x, cond, t, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, cond, t, context=c_crossattn)
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, cond=None, timesteps=None, context=None, y=None,**kwargs):
        ...
        if cond != None:
            ...
            h = torch.cat([self.zeros.repeat(h.size(0), 1, 1), h], dim=1)

        # TODO: take latent_res_h and latent_res_w as variables
        latent_res_h = res_h = 12
        latent_res_w = res_w = 16
        latent_res_t = t = 50
        h_xy = h[:, :, 0:latent_res_h*latent_res_w].view(h.size(0), h.size(1), latent_res_h, latent_res_w)
        h_yt = h[:, :, latent_res_h*latent_res_w:latent_res_w*(latent_res_h+latent_res_t)].view(h.size(0), h.size(1), latent_res_t, latent_res_w)
        h_xt = h[:, :, latent_res_w*(latent_res_h+latent_res_t):latent_res_w*latent_res_h+latent_res_t*latent_res_w+latent_res_t*latent_res_h].view(h.size(0), h.size(1), latent_res_t, latent_res_h)
        ...
        for module, input_attn in zip(self.input_blocks, self.input_attns):
            ...
            h_xy = h[:, :, 0:res_h*res_w].view(h.size(0), h.size(1), res_h, res_w)
            h_yt = h[:, :, res_h*res_w:res_w*(res_h+t)].view(h.size(0), h.size(1), t, res_w)
            h_xt = h[:, :, res_w*(res_h+t):res_w*res_h+t*res_w+t*res_h].view(h.size(0), h.size(1), t, res_h)
